...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-[#050505] pt-24 px-6">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-0 pointer-events-none select-none leading-none opacity-5">
        <h1 className="text-[22vw] uppercase text-white text-center tracking-tighter"
          style={{ fontFamily: "Anton, sans-serif" }}>{PERSONAL_INFO.name}
        </h1>
      </div>

      <div className="container mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center relative z-10 w-full">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}
          className="flex flex-col gap-6 lg:col-span-5 items-start text-left">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="text-6xl md:text-8xl font-extrabold uppercase text-white leading-none mt-2 drop-shadow-2xl"
            ...
            {mounted && (
              <AnimatePresence mode="wait">
                <motion.div key={roleIndex} initial={{ y: 40, opacity: 0 }}
                  ...
                  className="text-xl md:text-2xl font-bold text-[#c6ff00] whitespace-nowrap" >
                  {ROLES[roleIndex]}
                </motion.div>
              </AnimatePresence>
            )}
          ...
          <div className="grid grid-cols-2 gap-6 md:gap-10 mt-4 border-y border-white/5 py-6 w-full max-w-lg">
            <div>
          ...
          <div className="flex flex-wrap items-center gap-6 mt-4 w-full">
            <a href="#contact" className="px-6 py-3 bg-[#c6ff00] hover:bg-[#c6ff00]/90 text-black font-semibold text-sm rounded-xl transition-all duration-300 shadow-lg shadow-[#c6ff00]/10 hover:shadow-[#c6ff00]/25">
              Let's Connect
            </a>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div className="relative w-full aspect-[4/5] max-w-[480px] lg:max-w-[500px] rounded-3xl overflow-hidden border border-white/10 bg-white/5 backdrop-blur-md p-4 group">
            ...
            <div className="relative w-full h-full rounded-2xl overflow-hidden bg-neutral-900 border border-white/5">
              <img src="/images/Sachin.jpg" alt="Sachin" className="w-full h-full object-cover group-hover:scale-105 transition-all duration-700 ease-out"
                onError={(e) => {
