...
export default function Achievements() {
  ...
  return (
    <section id="achievements" className="py-32 px-6 bg-[#050505] relative overflow-hidden">
          ...
          {ACHIEVEMENTS.map((item, i) => {
            ...
            return (
              <motion.div key={item.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                className={`group relative p-8 rounded-3xl bg-[#0a0a0a] border border-white/5 hover:border-[#c6ff00]/30 transition-all duration-500 overflow-hidden flex flex-col justify-between min-h-[220px] ${isColSpan2 ? "md:col-span-2" : "md:col-span-1"}`} >
                ...
                <div className="relative z-10 mt-6 flex flex-col gap-2">
                  <h3 className="text-sm font-semibold tracking-wide uppercase text-neutral-500 font-mono">
                    {item.title}
                  </h3>
                  <p className="text-white font-extrabold text-3xl font-poppins leading-none tracking-tight">
                    {item.metric}
                  </p>
                  <p className="text-neutral-400 font-light text-sm leading-relaxed mt-1">
                    {item.detail}
                  </p>
                </div>
              </motion.div>
